...
const interfaceSettingKeys = [
    'enabled', 'hideNativeTopbar', 'dockNativePanels', 'motion', 'compactNav',
    'accent', 'density', 'radius', 'navWidth', 'chatWidth', 'messageWidth',
];
...
export class PreferencesManager {
    ...
    renderInterface() {
        ...
        return `<div class="nt-pref-interface">
          ...
          <section class="nt-pref-setting-group">
            <h3>${t('Layout')}</h3>
            ${toggle('enabled', 'Enable NastyTavern UI', 'Apply the NastyTavern interface layer.')}
            ${toggle('hideNativeTopbar', 'Replace native top toolbar', 'Use the NastyTavern application bar instead of the native top toolbar.')}
            ${toggle('dockNativePanels', 'Dock native panels', 'Integrate compatible SillyTavern panels into the NastyTavern workspace.')}
            ${toggle('compactNav', 'Compact navigation', 'Use the compact left navigation layout.')}
            ${range('navWidth', 'Navigation width', 'Width used when the navigation is expanded.', 180, 300, 2)}
            ${range('chatWidth', 'Chat max width', 'Maximum width of the main conversation surface.', 700, 1400, 20)}
            ${range('messageWidth', 'Message max width', 'Maximum width of individual messages.', 560, 1200, 20)}
          </section>
        </div>`;
    }
